...
pub const RootInteraction = interaction.RootInteraction;
...
/// RootInteraction entry for serialization (int-keyed)
pub const RootInteractionEntry = struct {
    id: i32,
    interaction: RootInteraction,
};

/// Serialize UpdateRootInteractions packet
...
/// - Inline variable: VarInt count + entries (key i32 + RootInteraction data)
pub fn serialize(
    allocator: std.mem.Allocator,
    update_type: serializer.UpdateType,
    max_id: i32,
    entries: ?[]const RootInteractionEntry,
) ![]u8 {
    ...
    if (entries) |ents| {
        ...
        // Each entry: i32 key + RootInteraction data
        for (ents) |entry| {
            // Key
            ...
            // RootInteraction data
            try entry.interaction.serialize(&buf, allocator);
        }
    }
...
test "UpdateRootInteractions with entries" {
    const allocator = std.testing.allocator;

    const entries = [_]RootInteractionEntry{
        .{ .id = 1, .interaction = .{} },
    };
