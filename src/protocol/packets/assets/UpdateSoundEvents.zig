/// UpdateSoundEvents Packet (ID 65)
///
/// Sends sound event definitions to the client.

const std = @import("std");
const serializer = @import("serializer.zig");
const sound_event = @import("../../../assets/types/sound_event.zig");

pub const SoundEventAsset = sound_event.SoundEventAsset;
pub const SoundEventLayer = sound_event.SoundEventLayer;

// Constants from Java UpdateSoundEvents.java
pub const PACKET_ID: u32 = 65;
...
/// SoundEvent entry for serialization
pub const SoundEventEntry = struct {
    id: u32,
    sound_event: SoundEventAsset,
};

/// Serialize UpdateSoundEvents packet
/// Format:
/// - nullBits (1 byte): bit 0 = soundEvents dictionary present
/// - type (1 byte): UpdateType enum
/// - maxId (4 bytes LE): maximum sound event ID
/// - If bit 0 set: VarInt count + for each: i32 key + SoundEvent data
pub fn serialize(
    ...
    if (entries.len > 0) {
        ...
        for (entries) |entry| {
            ...
            // SoundEvent data
            const se_data = try entry.sound_event.serialize(allocator);
            defer allocator.free(se_data);
            try buf.appendSlice(allocator, se_data);
        }
    }
...
test "UpdateSoundEvents with entries" {
    ...
    const entries = [_]SoundEventEntry{
        .{ .id = 0, .sound_event = .{ .id = "footstep", .volume = 0.8 } },
    };

    const pkt = try serialize(allocator, .init, 0, &entries);
    defer allocator.free(pkt);

    // Should have header + 1 sound event
