/// UpdateItemReticles Packet (ID 57)
///
/// Sends item reticle definitions to the client.

const std = @import("std");
const serializer = @import("serializer.zig");
const item_reticle = @import("../../../assets/types/item_reticle.zig");

pub const ItemReticleAsset = item_reticle.ItemReticleAsset;

// Constants from Java UpdateItemReticles.java
pub const PACKET_ID: u32 = 57;
pub const IS_COMPRESSED: bool = true;
pub const NULLABLE_BIT_FIELD_SIZE: u32 = 1;
pub const FIXED_BLOCK_SIZE: u32 = 6;
pub const VARIABLE_FIELD_COUNT: u32 = 1;
pub const VARIABLE_BLOCK_START: u32 = 6;
pub const MAX_SIZE: u32 = 1677721600;

/// ItemReticle entry for serialization
pub const ItemReticleEntry = struct {
    id: u32,
    reticle: ItemReticleAsset,
};

/// Serialize UpdateItemReticles packet
/// Format:
/// - nullBits (1 byte): bit 0 = itemReticles dictionary present
/// - type (1 byte): UpdateType enum
/// - maxId (4 bytes LE): maximum reticle ID
/// - If bit 0 set: VarInt count + for each: i32 key + ItemReticle data
pub fn serialize(
    allocator: std.mem.Allocator,
    update_type: serializer.UpdateType,
    max_id: u32,
    entries: ?[]const ItemReticleEntry,
) ![]u8 {
    var buf = std.ArrayListUnmanaged(u8){};
    errdefer buf.deinit(allocator);

    // nullBits: bit 0 = itemReticles present
    const null_bits: u8 = if (entries != null) 0x01 else 0x00;
    try buf.append(allocator, null_bits);

    // type (UpdateType)
    try buf.append(allocator, @intFromEnum(update_type));

    // maxId (i32 LE)
    var max_id_bytes: [4]u8 = undefined;
    std.mem.writeInt(i32, &max_id_bytes, @intCast(max_id), .little);
    try buf.appendSlice(allocator, &max_id_bytes);

    // itemReticles dictionary (if present)
    if (entries) |ents| {
        // VarInt count
        var vi_buf: [5]u8 = undefined;
        const vi_len = serializer.writeVarInt(&vi_buf, @intCast(ents.len));
        try buf.appendSlice(allocator, vi_buf[0..vi_len]);

        // Each entry: i32 key + ItemReticle data
        for (ents) |entry| {
            // Key (i32 LE)
            var key_bytes: [4]u8 = undefined;
            std.mem.writeInt(i32, &key_bytes, @intCast(entry.id), .little);
            try buf.appendSlice(allocator, &key_bytes);

            // ItemReticle data
            const reticle_data = try entry.reticle.serialize(allocator);
            defer allocator.free(reticle_data);
            try buf.appendSlice(allocator, reticle_data);
        }
    }

    return buf.toOwnedSlice(allocator);
}

test "UpdateItemReticles empty packet size" {
    const allocator = std.testing.allocator;
    const pkt = try serialize(allocator, .init, 0, null);
    defer allocator.free(pkt);
    try std.testing.expectEqual(@as(usize, 6), pkt.len);
}

test "UpdateItemReticles with entries" {
    const allocator = std.testing.allocator;

    const entries = [_]ItemReticleEntry{
        .{ .id = 0, .reticle = .{ .hide_base = false, .duration = 0.2 } },
    };

    const pkt = try serialize(allocator, .init, 0, &entries);
    defer allocator.free(pkt);

    // Should have header + 1 reticle
    try std.testing.expect(pkt.len > 7);

    // Check nullBits has dictionary present
    try std.testing.expectEqual(@as(u8, 0x01), pkt[0]);

    // Check type is init
    try std.testing.expectEqual(@as(u8, 0x00), pkt[1]);
}
