/// UpdateRepulsionConfig Packet (ID 75)
///
/// Sends repulsion configuration to the client.
/// Uses int-keyed dictionary.

const std = @import("std");
const serializer = @import("serializer.zig");
const repulsion_config = @import("../../../assets/types/repulsion_config.zig");

pub const RepulsionConfigAsset = repulsion_config.RepulsionConfigAsset;

// Constants from Java UpdateRepulsionConfig.java
pub const PACKET_ID: u32 = 75;
...
pub const MAX_SIZE: u32 = 65536011;

/// RepulsionConfig entry for serialization (int-keyed)
pub const RepulsionConfigEntry = struct {
    id: i32,
    config: RepulsionConfigAsset,
};

/// Serialize UpdateRepulsionConfig packet
/// Format (int-keyed dictionary):
/// - nullBits (1 byte): bit 0 = repulsionConfigs dictionary present
/// - type (1 byte): UpdateType enum
/// - maxId (4 bytes): i32 LE
/// - If bit 0 set: VarInt count + for each: i32 key + RepulsionConfig data (12 bytes)
pub fn serialize(
    allocator: std.mem.Allocator,
    update_type: serializer.UpdateType,
    max_id: i32,
    entries: []const RepulsionConfigEntry,
) ![]u8 {
    var buf = std.ArrayListUnmanaged(u8){};
    errdefer buf.deinit(allocator);

    // nullBits: bit 0 = repulsionConfigs present
    ...
    // repulsionConfigs dictionary (if present)
    if (entries.len > 0) {
        // VarInt count
        ...
        // Each entry: i32 key + RepulsionConfig data (12 bytes)
        for (entries) |entry| {
            // Key (i32 LE)
            ...
            // RepulsionConfig data (12 bytes inline)
...
test "UpdateRepulsionConfig empty packet size" {
...
test "UpdateRepulsionConfig with entries" {
    const allocator = std.testing.allocator;

    const entries = [_]RepulsionConfigEntry{
        .{ .id = 1, .config = .{ .radius = 1.0, .min_force = 0.5, .max_force = 2.0 } },
    };

    const pkt = try serialize(allocator, .init, 1, &entries);
    defer allocator.free(pkt);

    // Header (6 bytes) + VarInt(1) + 1 entry (4 + 12)
    // 6 + 1 + 16 = 23 bytes
    try std.testing.expectEqual(@as(usize, 23), pkt.len);
