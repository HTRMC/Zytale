/// UpdateBlockSoundSets Packet (ID 42)
///
/// Sends block sound set definitions to the client.

const std = @import("std");
const serializer = @import("serializer.zig");
const block_sound_set = @import("../../../assets/types/block_sound_set.zig");

pub const BlockSoundSetAsset = block_sound_set.BlockSoundSetAsset;
pub const BlockSoundEvent = block_sound_set.BlockSoundEvent;
pub const SoundEventIndexEntry = block_sound_set.SoundEventIndexEntry;

// Constants from Java UpdateBlockSoundSets.java
pub const PACKET_ID: u32 = 42;
...
/// BlockSoundSet entry for serialization
pub const BlockSoundSetEntry = struct {
    id: u32,
    sound_set: BlockSoundSetAsset,
};

/// Serialize UpdateBlockSoundSets packet
/// Format:
/// - nullBits (1 byte): bit 0 = blockSoundSets dictionary present
/// - type (1 byte): UpdateType enum
/// - maxId (4 bytes LE): maximum sound set ID
/// - If bit 0 set: VarInt count + for each: i32 key + BlockSoundSet data
pub fn serialize(
    allocator: std.mem.Allocator,
    update_type: serializer.UpdateType,
    max_id: u32,
    entries: []const BlockSoundSetEntry,
) ![]u8 {
    var buf = std.ArrayListUnmanaged(u8){};
    errdefer buf.deinit(allocator);

    // nullBits: bit 0 = blockSoundSets present
    ...
    // blockSoundSets dictionary (if present)
    if (entries.len > 0) {
        // VarInt count
        ...
        // Each entry: i32 key + BlockSoundSet data
        for (entries) |entry| {
            // Key (i32 LE)
...
test "UpdateBlockSoundSets empty packet size" {
...
test "UpdateBlockSoundSets with entries" {
    const allocator = std.testing.allocator;

    const entries = [_]BlockSoundSetEntry{
        .{ .id = 0, .sound_set = .{ .id = "grass_sounds" } },
    };
